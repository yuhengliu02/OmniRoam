...
def process_data_dict(data_dict, seen_prompts):
    """
    Process ODE pair data dict.
    
    Supports two formats:
    1. Legacy format: {prompt: video_tensor}
    2. Custom format: {
           "ode_latent": tensor,
           "trajectory": tensor,
           "speed": tensor,
    ...
    All data is preserved without deduplication.
    seen_prompts is used only for statistics: {video_id: count}
    """
    output_dict = {}

    if "ode_latent" in data_dict:
        prompt = data_dict["prompt"]
        video_id = data_dict.get("video_id", prompt)

        if video_id in seen_prompts:
            seen_prompts[video_id] += 1
        else:
            seen_prompts[video_id] = 1

        ode_latent = data_dict["ode_latent"].half().numpy()
        trajectory = data_dict["trajectory"].half().numpy()
        speed = data_dict["speed"].float().numpy()
        ...
        if "input_latent" in data_dict:
            input_latent = data_dict["input_latent"].half().numpy()
            result["input_latents"] = input_latent
